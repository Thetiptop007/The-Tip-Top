...
export default function UserDropdown() {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11 bg-indigo-100 flex items-center justify-center">
          <span className="text-indigo-600 font-semibold text-lg">
            {user?.name?.first?.charAt(0).toUpperCase() || user?.name?.charAt?.(0)?.toUpperCase() || 'A'}
          </span>
        </span>

        <span className="block mr-1 font-medium text-sm">
          {user?.name?.first ? `${user.name.first} ${user.name.last || ''}`.trim() : user?.name || 'Admin'}
        </span>
        <svg
      ...
      >
        <div>
          ...
          <span className="mt-0.5 block text-xs text-gray-500">
            {user?.email || 'admin@thetiptop.com'}
          </span>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-3 px-3 py-2 mt-3 font-medium text-red-600 rounded-lg group text-sm hover:bg-red-50 hover:text-red-700"
        >
          <svg
            className="fill-red-600 group-hover:fill-red-700"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
